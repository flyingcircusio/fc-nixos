  ...
  let
    ...
    makeFrrHost =
      ...
      let
        vlans = [
          idx
          (if idx == 4 then 1 else idx + 1)
        ];
        address = makeUnderlayAddress idx;
        export-filter = if redistribute then "accept-all-routes" else "accept-local-routes";
      in
      {
        imports = [
          baseConfig
    ...
    makeEvpnHost =
      ...
      let
        ...
        tapByName = builtins.listToAttrs ((lib.imap0 (i: v: lib.nameValuePair "tap${toString i}" v)) taps);
        ...
        makePingService =
          ...
          in
          lib.nameValuePair "ping-${iface}" (rec {
