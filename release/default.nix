...
let
  ...
  versionSuffix = (if stableBranch then "." else ".dev") + "${toString fc.revCount}.${shortRev}";
  ...
  combinedSources = pkgs.stdenv.mkDerivation {
    inherit fcSrc;
  ...
  initialEnv = if stableBranch then branch else "fc-25.11-dev";
  ...
  initialVMContents = [
    ...
    {
      source = (
        pkgs.writeText "fc-agent-initial-run" ''
          ...
          lib.traceValFn
            (
              ...
              if builtins.isAttrs v then
                "getDottedPackageNames in attrset with ${builtins.attrNames v}\n\n"
              else
                "getDottedPackageNames on ${toString v}\n\n"
            )
            (
              if lib.isDerivation v then
                dottedName
              else if shouldRecurse then
                getDottedPackageNames v attrPath
              else
                null
            )
        ) attrs)
  ...
  pkgNameToHydraJobs =
    ...
    in
    map (system: lib.setAttrByPath (path ++ [ system ]) job) supportedSystems;

  pkgNameListToHydraJobs =
    pkgNameList:
    # Merge the single-attribute sets from pkgNameToHydraJobs into one big attrset.
    lib.foldl' lib.recursiveUpdate { } (lib.flatten (map pkgNameToHydraJobs pkgNameList));
  ...
  channelsUpstream = lib.mapAttrs (
    ...
    let
      fullName =
        if (parseDrvName name).version != "" then
          "${src.name}.${substring 0 11 src.rev}"
        else
          "${src.name}-0.${substring 0 11 src.rev}";
    in
  ...
  images =
    ...
    in
    {
      ...
      fc =
        lib.hydraJob
          (import "${nixpkgs_}/nixos/lib/eval-config.nix" {
            inherit system;
            modules = [
              (import ./vm-image.nix imgArgs)
              (import version_nix { })
  ...
  release =
    ...
    pkgs.releaseTools.channel rec {
      ...
      constituents = [
        ...
        "images.*"
        "doc.roles"
        "importantPackages.*.x86_64-linux"
        "pkgs.*.x86_64-linux"
        "tests.*"
      ];
      preferLocalBuild = true;
