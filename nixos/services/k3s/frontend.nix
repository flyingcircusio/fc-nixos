...
let
  ...
  serviceListenConfigs = lib.mapAttrs (
    ...
    let
      ...
      binds =
        if (conf.binds != null) then
          conf.binds
        else
          let
            port =
              if conf.lbServicePort != null then
                conf.lbServicePort
              else if conf.servicePort != null then
                conf.servicePort
              else
                conf.podPort;
          in
        ...
        attrsOf (submodule {
          ...
          options = {
            ...
            binds = mkOption {
              ...
              description = ''
                Addresses with ports haproxy is binding to,
