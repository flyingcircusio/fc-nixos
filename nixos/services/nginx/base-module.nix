...
let
  ...
  configFile = (if cfg.validateConfigFile then writeNginxConfig else pkgs.writeText) "nginx.conf" ''
  ...
  '';
  nginxReloadMaster = pkgs.writeScriptBin "nginx-reload-master" ''
    #!${pkgs.runtimeShell} -e
...

{
  options = {
    services.nginx = {
      ...
      validateConfigFile = lib.mkEnableOption "validating configuration with writeNginxConfig" // {
        default = true;
      };
    };
  };
