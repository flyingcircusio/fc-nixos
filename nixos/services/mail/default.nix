...
let
  ...
  copyToStore =
    pathString:
    let
      p = builtins.path { path = pathString; };
    in
    "${p}";
  ...
  fclib = config.fclib;
  primaryDomainCandidates = attrNames (
    filterAttrs (domain: config: config.enable && config.primary) role.domains
  );
  primaryDomain =
    if primaryDomainCandidates != [ ] then head primaryDomainCandidates else "example.org";
  ...
  '';
  userAliases = concatMapStringsSep "\n" (
  ...
  config =
    ...
    lib.mkMerge [
      ...
      (lib.mkIf svc.enable {
        ...
        environment.etc = {
          "local/mail/config.json.example".text = (
            toJSON {
              domains = {
                "${primaryDomain}" = {
                  enable = true;
              ...
              dynamicMaps = {
                transport = [ "/etc/local/mail/transport" ];
              };
            }
          );
          ...
          "local/mail/users.json.example".text = (
            toJSON {
              "user@${primaryDomain}" = {
      ...
      (lib.mkIf (svc.enable && domains != [ ]) (
        ...
        in
        {
          assertions = [
            ...
            {
              assertion =
                role.mailHost != role.webmailHost || (role.imprintUrl == null && role.imprintText == null);
              message = ''
          ...
          environment = {
            etc = {
              ...
              "local/mail/local_valiases.json.example".text = (
                toJSON {
      ...
      (lib.mkIf (svc.enable && config.services.telegraf.enable && domains != [ ]) {
