...
in {
  options = with lib; {
    ...
    flyingcircus.services.postgresql = {
      ...
      autoUpgrade = {
        enable = mkEnableOption "Enable auto-upgrades when package major version changes.";
        ...
        expectedDatabases = mkOption {
          ...
          description = ''
            ...
            If unspecified databases are found, the upgrade is aborted.
          '';
        };
      };
  ...
  config = lib.mkMerge [
    ...
    in {
      ...
      systemd.services.postgresql.unitConfig = {
        ConditionPathExists = [
          ...
          "!${upstreamCfg.dataDir}/fcio_migrated_to"
          "!${upstreamCfg.dataDir}/fcio_upgrade_prepared"
        ];
      };
      ...
      in ''
        ...
        ln -sfT ${config.services.postgresql.package} ${config.services.postgresql.dataDir}/package
        ln -sfT ${config.services.postgresql.dataDir}/package /nix/var/nix/gcroots/per-user/postgres/package_${cfg.majorVersion}
      '';
    ...
    (lib.mkIf (cfg.autoUpgrade.enable && cfg.majorVersion != oldestMajorVersion ) {
      systemd.services.fc-postgresql-upgrade = {
        path = with pkgs; [
          sudo
        ];
