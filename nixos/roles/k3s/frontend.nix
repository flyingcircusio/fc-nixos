...
let
  fclib = config.fclib;
  cfg = config.flyingcircus.roles.k3s-frontend;
  master = fclib.findOneService "k3s-server-server";
...
{
  options = with lib; {
    flyingcircus.roles.k3s-frontend = {
      enable = lib.mkEnableOption "Enable K3s (Kubernetes) Frontend (experimental)";
  ...
  config = lib.mkMerge [
      ...
      assertions = [
        {
          ...
          message = "Invalid cluster configuration: No K3s server found!";
        }
      ];
    ...
    (lib.mkIf (cfg.enable && !masterRoleEnabled && !nodeRoleEnabled) {
      ...
      flyingcircus.activationScripts.kubernetes-apitoken-node = ''
        ...
        umask 077
        echo ${master.password} | md5sum | head -c64 > /var/lib/kubernetes/secrets/apitoken.secret
      '';
