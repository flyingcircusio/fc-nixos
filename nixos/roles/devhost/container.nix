...
let
  ...
  testedRoles = attrNames (lib.filterAttrs (n: v: v.supportsContainers or true)
    config.flyingcircus.roles);
  excludedRoles = attrNames
    (lib.filterAttrs (n: v: !(v.supportsContainers or true))
      config.flyingcircus.roles);

  containersFromDir = dir:
    with lib;
    (map (filename: fromJSON (readFile "${dir}/${filename}"))
      (filter (filename: hasSuffix ".json" filename)
        (attrNames (readDirMaybe dir))));

  allContainers = (containersFromDir "/etc/devhost")
  ...
  makeContainerTestScriptForRole = (role:
    pkgs.writeShellScript "check_devhost_with_${role}" ''
      #!/bin/sh
      set -euxo pipefail
    ...
    '');
in lib.mkMerge [
  (lib.mkIf (cfg.enable && cfg.virtualisationType == "container") {
    environment.systemPackages = [ container_script clean_script ];

    security.sudo.extraRules = lib.mkAfter [{
      commands = [{
        command = "${container_script}/bin/fc-build-dev-container";
        options = [ "NOPASSWD" ];
      }];
      groups = [ "service" "users" ];
    }];

    systemd.tmpfiles.rules = [ "d /etc/devhost/" ];
    ...
    services.nginx.virtualHosts = if cfg.enableAliasProxy then
      (let
        ...
        containers =
          filter (container: container.aliases != [ ]) enabledContainers;
        generateContainerVhost = container: {
          name = "${container.name}.${suffix}";
          value = {
            serverAliases = map (alias: "${alias}.${container.name}.${suffix}")
              container.aliases;
            forceSSL = true;
            enableACME = true;
            locations."/" = { proxyPass = "https://${container.name}"; };
          };
        };
      in builtins.listToAttrs (map generateContainerVhost containers))
    else
      { };

    systemd.services."container@".serviceConfig = {
      TimeoutStartSec = lib.mkForce "1min";
    };
    ...
    systemd.timers.fc-devhost-clean-containers =
      lib.mkIf cfg.cleanupContainers {
  ...
  (lib.mkIf cfg.testing {
    ...
    flyingcircus.services.sensu-client.checks = listToAttrs (map (role: {
      ...
      value = {
        ...
        command = (let
          check_command = pkgs.writeShellScript "check" ''
            set -e
