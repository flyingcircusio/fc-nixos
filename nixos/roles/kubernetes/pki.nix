    ...
    services.certmgr = {
      ...
      specs =
        let
          ...
          # Certmgr 3.0 doesn't like it when multiple specs specify that the CA
          # should be written to disk at the same place. Creating a separate spec
          # for the CA breaks the Certmgr's parser, so we add the CA part only to
          # the first cert spec we get.
          firstCertName = head (attrNames cfg.certs);
          firstCertWithCa = certSpecs.${firstCertName} // { ca.path = top.caFile; };
          certSpecs = mapAttrs mkSpec cfg.certs;
        in certSpecs // { ${firstCertName} = firstCertWithCa; };
