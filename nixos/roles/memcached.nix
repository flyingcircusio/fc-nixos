...
let
  ...
  localConfig = fclib.jsonFromFile "/etc/local/memcached/memcached.json" defaultConfig;
    ...
    (mkIf cfg.enable {
      ...
      flyingcircus.services = {
        sensu-client.checks.memcached =
          let
            host =
              if localConfig ? listen then elemAt (lib.splitString "," localConfig.listen) 0 else "localhost";
          in
          {
