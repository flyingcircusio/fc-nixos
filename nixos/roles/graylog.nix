  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.enable {
      ...
      networking.firewall.extraCommands = ''
        ip46tables -A nixos-fw -i ${fclib.network.srv.interface} -p udp --dport ${toString syslogInputPort} -j nixos-fw-accept
        ip46tables -A nixos-fw -i ${fclib.network.srv.interface} -p tcp --dport ${toString beatsTCPHAPort} -j nixos-fw-accept
      '';
