...
let
  ...
  formatList = list: "[ ${lib.concatMapStringsSep " " (n: ''"${n}"'') list} ]";

  esVersion =
    if config.flyingcircus.roles.elasticsearch6.enable then
      "6"
    else if config.flyingcircus.roles.elasticsearch7.enable then
      "7"
    else
      null;
  ...
  esServices =
    (fclib.findServices "elasticsearch6-node") ++ (fclib.findServices "elasticsearch7-node");

  defaultEsNodes = map (service: head (lib.splitString "." service.address)) esServices;
  ...
  configFromLocalConfigDir = fclib.configFromFile "${localConfigDir}/elasticsearch.yml" "";
