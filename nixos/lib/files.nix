...
rec {
  ...
  filesRel =
    path:
    optionals (pathExists path) (
      attrNames (
        filterAttrs (filename: type: (type == "regular" || type == "symlink")) (builtins.readDir path)
      )
    );

  # Get all regular files and symlinks with their absolute name
  files = path: (map (filename: (builtins.toString path) + ("/" + filename)) (filesRel path));

  # Reads the config file if it exists, else returns predefined default
  configFromFile = file: default: if pathExists file then readFile file else default;

  jsonFromFile = file: default: builtins.fromJSON (configFromFile file default);
  ...
  jsonFromDir =
    path:
    let
      objects = map (filename: builtins.fromJSON (readFile filename)) (
