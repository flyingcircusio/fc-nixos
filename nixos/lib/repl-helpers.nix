...
let
  printEtcFile =
    ...
    let
      ...
      content = if value.text != null then print value.text else print (readFile value.source);
    in
    content;
  ...
  print =
    ...
    in
    trace formatted (
      if isString formatted then "output hash: " + (hashString "sha256" formatted) else 0
    );
