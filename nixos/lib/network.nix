...
let
  ...
  foldConds =
    ...
    in
    lib.foldl f value conds;

  interfaceData =
    let
      underlayInterfaces = lib.filterAttrs (
        name: value: name != "ul" && value.policy or null == "underlay"
      ) encInterfaces;
    ...
    let
      ...
      go =
        ...
        let
          ...
          moduloOffset = if (lib.mod capacity remaining) >= (length state) then 1 else 0;

          prefixLength = prefixBaseLen + moduloOffset;
        in
        [ (lib.substring 0 prefixLength item) ] ++ state;

      builder = lib.foldr go [ ] parts;
    in
    prefix + (lib.concatStringsSep "-" builder);
...
rec {
  ...
  netmaskFromPrefixLength =
    prefix: (ip4.fromNumber (((fclib.pow 2 prefix) - 1) * (fclib.pow 2 (32 - prefix))) prefix).address;
  netmaskFromCIDR = cidr: netmaskFromPrefixLength (prefixLength cidr);
  ...
  listServiceAddresses =
    service:
    (map (service: service.address) (filter (s: s.service == service) config.flyingcircus.encServices));

  listServiceIPs =
    service:
    (lib.flatten (
      map (service: service.ips) (filter (s: s.service == service) config.flyingcircus.encServices)
    ));
  ...
  listServiceAddressesWithPort =
    service: port: map (address: "${address}:${toString port}") (listServiceAddresses config service);
  ...
  routingPriority = vlan: if hasAttr vlan routingPriorities then routingPriorities.${vlan} else 100;

  # "example.org." -> absolute name; "example" -> relative to $domain
  normalizeDomain =
    domain: n: if lib.hasSuffix "." n then lib.removeSuffix "." n else "${n}.${domain}";
    ...
    lib.mapAttrs' (
      ...
      lib.nameValuePair vlan (
        let
          ...
          mtu =
            if hasAttr vlan config.flyingcircus.static.mtus then
              config.flyingcircus.static.mtus.${vlan}
            else
              1500;
        in
        with fclib;
        rec {
          ...
          link =
            if policy == "underlay" then
              underlayLoopbackLinkName
            else if policy == "vxlan" then
              "vx${vlan}"
            else if policy == "tagged" then
              "${buildComplexInterfaceName "eth-" (builtins.head interface'.nics).external_label or "vlan"}"
            else
              taggedLink;

          externalLabel =
            if (length (interface'.nics or [ ]) > 0) then
              (builtins.head interface'.nics).external_label
            else
              null;

          linkStack = lib.unique (
            filter (l: l != null) [
              link
          ...
          mac = lib.toLower (lib.attrByPath [ "mac" ] macFallback interface');
          ...
          dualstack = rec {
            ...
            networkAttrs = lib.mapAttrsToList (network: addresses: {
            ...
            attrs = lib.flatten (
              lib.mapAttrsToList (
                network: addresses:
                let
                  prefix = fclib.prefixLength network;
                in
                (map (address: {
                  address = address;
                  prefixLength = prefix;
            ...
            defaultGateways = lib.mapAttrsToList (network: gateway: gateway) (
              lib.filterAttrs (
                network: gateway: (length interface'.networks.${network} >= 1) && (priority < 100)
              ) interface'.gateways
            );
          };
    ...
    then
      ...
      let
        ...
        underlayMacFallback =
          ...
          in
          "ul-mac-" + (lib.concatStrings digits);

        emptyLabels = any (l: l.external_label == "") links;
        uniqueNames =
          length links == length (lib.unique (map (l: underlayLinkName l.external_label) links));

        makeName =
          if emptyLabels || !uniqueNames then
            (l: underlayMacFallback l.mac)
          else
            (l: underlayLinkName l.external_label);
      in
      {
        asNumber = 4200000000 + hostId;
        loopback =
          let
            addrs = network.ul.v4.addresses;
          in
          if length addrs == 0 then throw "Underlay network has no address assigned" else head addrs;
