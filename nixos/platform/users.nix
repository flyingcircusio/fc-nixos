...
let
    ...
    lib.listToAttrs (
      map (user: {
        ...
        value = {
          ...
          createHome = false;
          extraGroups = lib.optionals (user.class == "service") serviceUserExtraGroups;
  ...
  groupMembershipsFor =
    user:
    if currentRG != null && lib.hasAttr currentRG user.permissions then
      lib.listToAttrs (
        map
  ...
  permissionGroups =
    permissions:
    lib.listToAttrs (
      filter (group: group.name != "wheel") # This group already exists
        (
          map (permission: {
  ...
  mergeSets =
    ...
    if (length listOfSets) == 1 then
      ...
      lib.recursiveUpdate (head listOfSets) (mergeSets (tail listOfSets));

  htpasswdUsers =
    group:
    lib.optionalString (config.users.groups ? "${group}") (
      concatStringsSep "\n" (
        map (user: "${user.name}:${user.hashedPassword}") (
          filter (user: (user.hashedPassword != null) && (stringLength user.hashedPassword) > 0) (
            map (username: config.users.users.${username}) (config.users.groups."${group}".members)
          )
        )
      )
    );
...
in
{
  ...
  config = {
    ...
    environment.etc."local/htpasswd_fcio_users.login".text = htpasswdUsers "login";
    environment.etc."local/htpasswd_fcio_users.manager".text = htpasswdUsers "manager";
    environment.etc."local/htpasswd_fcio_users.sudo-srv".text = htpasswdUsers "sudo-srv";
    environment.etc."local/htpasswd_fcio_users.wheel".text = htpasswdUsers "wheel";
    ...
    flyingcircus.users = with lib; {
      ...
      permissions = mkDefault (
        fclib.jsonFromFile cfg.permissionsPath ''
          [
           {
    ...
    flyingcircus.activationScripts.homedirs = lib.stringAfter [ "users" ] (
