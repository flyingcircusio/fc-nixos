...
let
  ...
  sensuServer = findFirst (s: s.service == "sensuserver-server") null config.flyingcircus.encServices;
  ...
  encTags = listToAttrs (
    filter
      ...
      (tag: ((tag.name != "fc_component") && (tag.name != "fc_role") && (tag.name != "env")))
      (
        map (split: nameValuePair (elemAt split 0) (elemAt split 1)) (
  ...
  globalTags =
    encTags
    // (optionalAttrs (params ? resource_group) { resource_group = params.resource_group; })
    // (optionalAttrs (params ? profile) { profile = params.profile; });
    ...
    (mkIf config.services.telegraf.enable {
      ...
      flyingcircus.services.sensu-client.checks = {
        telegraf_prometheus_output = {
          ...
          command =
            "check_http -v -j HEAD -H ${config.networking.hostName} " + "-p ${telegrafPort} -u /metrics";
        };
      };
