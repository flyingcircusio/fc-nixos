...
Service users may invoke `sudo systemctl` to restart individual
...
See also [nixos-local](../../platform-releases/fc-26.05-production/local.md#nixos-local) for information about how to activate system
...
[nixos-systemd-app-service-example](../../platform-releases/fc-26.05-production/systemd.md#nixos-systemd-app-service-example) below.


## Custom Units

You can define your own unit files using NixOS configuration modules
in `/etc/local/nixos` or plain unit files in `/etc/local/systemd`.
...
This starts the service after boot and when `fc-manage` is run.
It runs as user *s-myservice* and doesn't restart if the executable fails.

#### Application Service Example { #nixos-systemd-app-service-example }
...
We still support plain unit config in in `/etc/local/systemd/<unit-name>.service`
but it's deprecated. Use Nix config instead, as shown above.

We bind your service unit to the `multi-user.target` by default so they
