# User Package Management { #nixos-user-package-management }
...
## Custom User Environments { #user-env }
...
The URL for the current release can be found in the [changelog](../../changes/index.md)
...
A `result` symlink now points to the generated environment. It can be
...
This picks up changes in `myproject_env.nix` and package updates
...
let
  pkgs = import <nixpkgs> {};
  pkgsUnstable = import (fetchTarball https://releases.nixos.org/nixos/unstable/nixos-25.05beta738172.eb62e6aa39ea/nixexprs.tar.xz) {};
in
pkgs.buildEnv {
  name = "myproject-env";
  paths = with pkgs; [
...
This installs the `zlib` from the platform NixOS version but `libjpeg` from NixOS unstable (here 25.05pre).
