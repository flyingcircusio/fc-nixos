# Garbage collection & fc-userscan { #nixos-gc-fc-userscan }
...
Also see `man fc-userscan` for more information about the tool.
...
You can also run `sudo fc-collect-garbage` (available for *service*
...
default set of excludes in `/etc/userscan/exclude` which is used
...
to `~/.userscan-ignore`. The file uses a pattern format like
