...
[nixos-postgresql-server](../components/postgresql.md#nixos-postgresql-server) is used as a database to store Roundcube settings.
...
## How do I perform a basic setup? { #nixos-mailserver-basic-setup }
...
Create a configuration file like `/etc/local/nixos/mail-config.nix` which
...
Run `sudo fc-manage switch` to have everything configured on the system.

After running the above command, a newly-generated file `/etc/local/mail/dns.zone`
...
Edit the file `/etc/local/mail/users.json` to add user accounts. Example:
...
in the corresponding configuration file, e.g. `/etc/local/nixos/mail-config.nix`
and the password must be hashed via `mkpasswd -m yescrypt {PASSWORD}`.
...
**hashedPassword** option in `/etc/local/mail/users.json` empty and set
the initial password in `/var/lib/dovecot/passwd` instead. This file
...
The initial password hash can be created with `mkpasswd -m yescrypt {PASSWORD}`, as shown above. Afterwards, user can log into the Roundcube web mail
...
## How do I forward mails to remote addresses? { #mail-into-backends }
...
file `/etc/local/nixos/mail-config.nix`:

```
flyingcircus.roles.mailserver.dynamicMaps.virtual_alias_maps = ["/etc/local/mail/virtual_aliases"];
```

Then, create the file `/etc/local/mail/virtual_aliases` and define your aliases.
Example contents:
...
Invoke `sudo systemctl reload postfix` to recompile the maps after the map
contents has been changed. Invoke `sudo fc-manage switch` as usual if
the content of the NixOS module `/etc/local/nixos/mail-config.nix` have been changed.
...
Invoke `sudo systemctl reload postfix` to recompile maps after map
contents has been changed. Invoke `sudo fc-manage switch` as usual if
...
: *mail domains* which should be served by this mail server.
  ...
  You can find these options below. See [nixos-mailserver-basic-setup](../components/mailserver.md#nixos-mailserver-basic-setup)
...
: Hash map of Postfix maps (like [transport]) and one or more file paths
  containing map records. See section [mail-into-backends](../components/mailserver.md#mail-into-backends) for details.
...
: Virtual mail users listing in `passwd(7)` format. Set this if an
...
: Either a salted SHA-256 password hash (for static passwords) or empty string.
  In the latter case, the password is read from `/var/lib/dovecot/passwd`.
