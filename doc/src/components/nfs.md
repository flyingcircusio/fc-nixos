...
`/etc/exports` for the NFS server and `/etc/fstab` for the NFS
clients.
...
All NFS clients mount the NFS share at `/mnt/nfs/shared`. This directory is
...
The NFS server stores its data at `/srv/nfs/shared`. This directory is also
