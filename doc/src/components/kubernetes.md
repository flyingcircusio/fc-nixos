...
This platform release defaults to using k3s version 1.33.x.
...
The `kubectl` command can be used on the K3s server directly after
login via SSH. The `helm` command is also available there.
...
in the current directory by running `kubernetes-make-kubeconfig`:
...
generated by `kubernetes-make-kubeconfig` as described above.
Kubeconfig files for the dashboard must contain a valid service account token.

### Cheat sheet

This is a shortened version of `kubectl` commands, you can look up the
...
with each other on the [SRV network](../infrastructure/networking/networking.md#logical-networks) and
...
- Nginx listening on the public
  [FE interface](../infrastructure/networking/networking.md#logical-networks),
...
See [nixos-webgateway](../components/webgateway.md#nixos-webgateway) for more details about general HAProxy and nginx
configuration.
...
- HAProxy in *tcp* mode, listening on the public
  [FE interface](../infrastructure/networking/networking.md#logical-networks), forwarding requests from the Internet to Kubernetes ingress.
...
### Updating k3s { #nixos-k3s-update-versions }
