# PostgreSQL { #nixos-postgresql-server }

Managed instance of the [PostgreSQL](http://postgresql.org) database server.

## Components

- PostgreSQL server (versions 14, 15, 16, 17, 18)
...
`services.postgresql.settings` option in a custom NixOS module.
Place it in `/etc/local/nixos/postgresql.nix`, for example:
...
To override platform defaults, use `lib.mkForce` before the wanted value
...
Service users can use `sudo -u postgres -i` to access the
PostgreSQL superuser account to perform administrative commands like
`createdb` and `createuser`.

Service users may invoke `sudo fc-manage switch`
...
- `postgresql-alive`: uses the UNIX socket in `/run/postgresql` to
...
`fcio_monitoring` database user. Metrics can be viewed when a [nixos-statshost](../components/statshost.md#nixos-statshost)
...
## Major Version Upgrades { #nixos-postgresql-major-upgrade }

Upgrading to a new major version, for example from 14.x to 15.x, requires a
migration of the old database cluster living in `/srv/postgresql/14` to
a new data directory at `/srv/postgresql/15`. A common way to do this
...
!!! note
    `fc-postgresql` has to be run as `postgres` user. Prefix the
...
!!! note
    Please look at the output of `sudo -u postgres fc-postgresql list-versions`
...
Run `sudo -u postgres fc-postgresql list-versions` to see how the
...
as `postgres` user, remove the `fcio_migrated_to*` files in the old data
