...
*ethsrv*-interfaces of your VM on port 6379 (See [networking](../infrastructure/networking/networking.md) for details on this topic).

In previous versions, custom redis configuration could be set
via `/etc/local/redis/custom.conf` which is not supported anymore.
...
Service users may invoke `sudo fc-manage switch` to apply
