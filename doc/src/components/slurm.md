# Slurm Workload Manager { #nixos-slurm }
...
!!! note
    ...
    be set manually. See the [nixos-slurm-config-reference](../components/slurm.md#nixos-slurm-config-reference) on how to do that.
...
[nixos-statshost](../components/statshost.md#nixos-statshost) and displayed using a Grafana dashboard.
...
[nixos-mysql](../components/mysql.md#nixos-mysql) database with our platform defaults and
...
!!! warning
    ...
    [nixos-fc-slurm](../components/slurm.md#nixos-fc-slurm).
...
## Managing clusters with the fc-slurm command { #nixos-fc-slurm }

Use `fc-slurm` to manage the state of slurm compute nodes and display
...
## Upgrading the Slurm cluster { #nixos-slurm-upgrade }
