...
class Manager
{
    ...
    public static function initPluginDirectories()
    {
        ...
        var_dump($GLOBALS['MATOMO_PLUGIN_DIRS']);
        print "\n";

        if (!empty($GLOBALS['MATOMO_PLUGIN_DIRS'])) {
            foreach ($GLOBALS['MATOMO_PLUGIN_DIRS'] as $pluginDir => &$settings) {
                if (!isset($settings['pluginsPathAbsolute'])) {
                    throw new \Exception('Missing "pluginsPathAbsolute" configuration for plugin dir $settings');
                }
                if (!isset($settings['webrootDirRelativeToMatomo'])) {
                    throw new \Exception('Missing "webrootDirRelativeToMatomo" configuration for plugin dir' . json_encode($settings));
                }
            }
