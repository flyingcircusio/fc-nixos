...
*Version* Major version of PostgreSQL. All versions available on the current
platform version are shown.
*Data Dir* Path to data dir if it exists for this version.
*Service Running?* `Yes` if postgresql.service is running and is using this
version.
*Migrated To* Target data dir where data has been migrated to. Data dirs with
which have been migrated must not be used anymore.
*Migrated From* source data dir where data has been migrated from.
points to the PostgreSQL version that is or has been used for this data dir.
*Package Known?* `Yes` if package link in data dir points is valid. The link
The link is needed to upgrade to a newer version as pg_upgrade needs the old
binaries to work. fc-postgresql migration commands and autoupgrade will fail
if this is missing.
*Prepared For Upgrade?* `Yes` if data dir is prepared to be used as target
for an upgrade migration. This means that `fc-postgresql prepare-autoupgrade`
or `fc-postgresql upgrade` without `--upgrade-now` has created this data dir.
