...
def fake_directory():
    ...
    d.deletions.return_value = collections.OrderedDict([
        ...
        ('node02', {
            'stages': ['prepare', 'soft']}),
        ('node03', {
            'stages': ['prepare', 'soft', 'hard']}),
        ('node04', {
            'stages': ['prepare', 'soft', 'hard', 'purge']}), ])
    return d


@pytest.fixture
def cluster(monkeypatch):
    monkeypatch.setattr(fc.ceph.api.cluster.Cluster, 'rbd',
...
def pools(cluster, monkeypatch):
    monkeypatch.setattr(fc.ceph.api.pools.Pools, 'names',
    ...
    monkeypatch.setattr(
        fc.ceph.api.pools.Pool, 'load', lambda self: images_hdd
        if self.name == 'rbd.hdd' else images_ssd)
    return fc.ceph.api.pools.Pools(cluster)


def test_node_deletion(fake_directory, cluster, pools):
    v = fc.ceph.maintenance.VolumeDeletions(fake_directory, cluster)
    v.ensure()
