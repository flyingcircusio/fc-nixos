...
class HAProxyLog(nagiosplugin.Resource):
    ...
    def __init__(self, logfile, statefile, percentiles, url_filters, exclude_patterns):
        self.logfile = logfile
        self.statefile = statefile
        self.percentiles = percentiles
        self.url_filters = url_filters or {None: ''}
        if exclude_patterns:
            exclude_patterns = list(map(lambda x: '({})'.format(x), exclude_patterns))
    ...
    def parse(self):
        ...
        with nagiosplugin.LogTail(self.logfile, cookie) as lf:
            for line in lf:
                ...
                if self.r_exclude.search(line.decode('iso-8859-1')):
                    logging.debug('hit exclude pattern in line: %s', line.strip())
                    continue
        ...
        return numpy.array(records, dtype=[
            ('t_tot', numpy.int32), ('err', numpy.uint16), ('url', '80a')])
    ...
    def request_rate(self, records):
        ...
        with nagiosplugin.Cookie(self.statefile) as cookie:
            ...
            if not last_run:
                ...
                return nagiosplugin.Metric('rate', 0, 'req/s', min=0,
                                           context='default')
            last_run = datetime.datetime(*last_run)
            timedelta = max((now - last_run).total_seconds(), 1)
            return nagiosplugin.Metric('rate', len(records) / timedelta,
                                       'req/s', min=0, context='default')
        ...
        if requests:
            for pct in self.percentiles:
                yield nagiosplugin.Metric(
                    name('t_tot%s' % pct),
                    numpy.percentile(records['t_tot'], int(pct)) / 1000, 's',
                    min=0, context='t_tot%s' % pct)
        else:
            logging.info('no requests found%s - skipping timing metrics',
                         ' for ' + label if label else '')
        errors = 100 * numpy.sum(records['err'] / requests) if requests else 0
        yield nagiosplugin.Metric(name('http_errors'), errors, '%', 0, 100,
                                  context='http_errors')
        yield nagiosplugin.Metric(name('requests'), requests, min=0,
                                  context='default')
...
options are given, all requests are counted.""")
    ...
    argp.add_argument('-s', '--state-file', default='check_haproxy_log.state',
                      ...
                      '(default: "%(default)s")')
    argp.add_argument('-f', '--filter', metavar='LABEL:URLPREFIX',
                      action='append', default=[],
                      help='filter URLs into labeled buckets and compute '
                      'statistics for each bucket')
    argp.add_argument('-e', '--exclude', metavar='PATTERN',
                      action='append', default=[],
...
def main():
    ...
    check = nagiosplugin.Check(
        HAProxyLog(args.logfile, args.state_file, percentiles, url_filters, exclude_patterns),
    ...
    for pct, i in zip(percentiles, itertools.count()):
        check.add(nagiosplugin.ScalarContext(
            't_tot%s' % pct, args.tw[i], args.tc[i],
