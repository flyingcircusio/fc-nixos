...
def main():
    ...
    if set(actual) != set(critical):
        ...
        print(
            "WARNING - Pressure Stalling Check encountered unexpected data, or we don't have some critical thresholds for the values we got"
        )
        print("Values I read: ", actual)

    for extent in actual:
        if extent in critical and threshold_exceeded(
            critical[extent], actual[extent]
        ):
        ...
        if extent in warning and threshold_exceeded(
            warning[extent], actual[extent]
        ):
            exit_code = max(exit_code, 1)
            print(
