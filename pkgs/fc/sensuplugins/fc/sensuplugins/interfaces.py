...
class Interfaces(nagiosplugin.Resource):
    ...
    def query(self, iface):
        ...
        if m and m.group(1) == 'yes':
            ...
            if m:
                ...
                raise RuntimeError('cannot parse ethtool output for speed:\n' +
                                   stdout)
        else:
            speed = 0
        ...
        if m:
            ...
            raise RuntimeError('cannot parse ethtool output for duplex:\n' +
                               stdout)
    ...
    def probe(self):
        ...
        for iface, context in self.ifaces.items():
            ...
            yield nagiosplugin.Metric('{}_spd'.format(iface),
                                      speed,
                                      'Mb/s',
                                      min=0,
                                      context='{}_spd'.format(context))
            yield nagiosplugin.Metric('{}_dup'.format(iface),
                                      duplex,
...
def main():
    ...
    argp.add_argument('-s',
                      '--speed',
                      default='1000:1000',
                      help='require at least SPEED Mb/s (default: '
                      '%(default)s)')
    argp.add_argument('-d',
                      '--duplex',
                      default='full',
    ...
    check = nagiosplugin.Check(
        Interfaces([i.split(',')[0] for i in args.interfaces],
                   args.auto_detect, args.exclude), InterfacesSummary())
