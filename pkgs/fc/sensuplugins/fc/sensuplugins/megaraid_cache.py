...
class MegaRAIDCache(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        yield nagiosplugin.Metric('vd{}_policy_diff'.format(self.diskid),
                                  len(differences) // 2,
                                  min=0,
                                  context='policy_diff')

    def __repr__(self):
        return '{}({}, {})'.format(self.__class__.__name__, self.diskid,
                                   self.ld.config)
    ...
    def probe(self):
        for line in self.ld.config:
            ...
            if m:
                ...
                continue
        return [
            nagiosplugin.Metric('vd{}_error_count'.format(self.vd),
                                self.errors,
                                min=0,
                                context='error_count'),
            nagiosplugin.Metric('vd{}_predictive_failure'.format(self.vd),
                                self.predictive,
                                min=0,
                                context='error_count'),
        ]
...
class BatteryReplace(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        try:
            stdout = subprocess.check_output([self.command],
                                             shell=True).decode()
    ...
    def evaluate(self, metric, resource):
        if metric.value == 'yes':
            return nagiosplugin.Result(
                nagiosplugin.Critical, '{}: {}'.format(metric.name,
                                                       metric.value), metric)
    ...
    def problem(self, results):
        ...
        for result in results.most_significant:
            ...
            if isinstance(r, MegaRAIDCache):
                msg.append('VD {}: missing {}, unexpected {}'.format(
...
def parse_args():
    ...
    a.add_argument('-t',
                   '--timeout',
                   default=20,
                   help='abort execution after '
                   'TIMEOUT seconds (default: %(default)s)')
    a.add_argument('-v',
                   '--verbose',
                   default=0,
                   action='count',
...
def main():
    ...
    check = nagiosplugin.Check(
        ...
        nagiosplugin.ScalarContext('error_count', args.errors_warning,
                                   args.errors_critical),
        nagiosplugin.ScalarContext('predictive_failure',
                                   args.predictive_failure_warn,
                                   args.predictive_failure_crit),
        BatteryReplaceContext('battery_replacement'), MegaRAIDSummary())
