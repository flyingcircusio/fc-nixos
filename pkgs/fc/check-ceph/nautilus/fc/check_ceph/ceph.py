...
class CephStatus(object):
    ...
    def summary(self):
        ...
        # There is no summary in luminous anymore. Instead try to build a string
        # containing everything that has been matched on in earlier versions.
        # long-term: create a list of necessary information and revamp extraction +
        # processing

        res = ", ".join(
            f"{checkname}: [{checkresult['severity']}] {checkresult['summary']}"
            for (checkname, checkresult) in self.status["health"][
                "checks"
            ].items()
        )
        return res
...
class Ceph(nagiosplugin.Resource):
    ...
    def probe(self):
        ...
        yield nagiosplugin.Metric("health", self.stat.overall_status)
