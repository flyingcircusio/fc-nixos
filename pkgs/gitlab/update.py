...
class GitLabRepo:
    ...
    def tags(self) -> Iterable[str]:
        ...
        versions.sort(
            key=lambda x: LooseVersion(x.replace("v", "").replace("-ee", "")),
            reverse=True,
        )
        return versions
...
@click.option(
    "--rev", default="latest", help="The rev to use (vX.Y.Z-ee), or 'latest'"
)
def update_data(rev: str):
    ...
    if rev == "latest":
        ...
        rev = next(
            filter(lambda x: not ("rc" in x or x.endswith("pre")), repo.tags)
        )
